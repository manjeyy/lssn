...
const ACCESS_KEY = 'lssn_admin_access_token';
const REFRESH_KEY = 'lssn_admin_refresh_token';
...
export async function updateUserRole(id: number, role: 'viewer' | 'creator' | 'admin') {
...
export async function getLssnsAll() {
  return apiFetch<any[]>('/lssns?all=true');
}
...
export async function updateCategory(id: number, payload: unknown) {
...
export async function updateTopic(id: number, payload: unknown) {
  return apiFetch<any>(`/topics/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
